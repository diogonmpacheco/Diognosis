...
function publicDisplayText(value, fallback = "") {
  ...
  text = text
    ...
    .replace(/\bpending\s+(?:source linked\s+)?(?:professional\s+)?(?:clinical\s+)?review\b/gi, "professional sign-off not claimed")
    .replace(/\bpending\s+[\w/-]+(?:\s+[\w/-]+){0,4}\s+review\b/gi, "professional sign-off not claimed")
    .replace(/\bpending[-_\s]?review\b/gi, "professional sign-off not claimed")
