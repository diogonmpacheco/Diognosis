...
const PENDING_LIVE_RECEPTOR_AUGMENTATION = (() => {
  const summary = {
    schema:"diognosis.pending-live-receptor-augmentation.v1",
    receptorProfilesAdded:0,
    receptorSignalsAttached:0,
  ...
  const existing = new Set(Object.keys(RECEPTOR_SCORES || {}).map(pendingReceptorCompareKey));
  for (const candidate of data.receptorPhenotypeCandidates || []) {
    const drugName = pendingReceptorPrimaryDrugName(candidate);
    if (!drugName) continue;
    const key = pendingReceptorLookupKey(drugName);
    if (!key) continue;
    const existingKey = Object.keys(RECEPTOR_SCORES || {}).find(scoreKey => pendingReceptorCompareKey(scoreKey) === pendingReceptorCompareKey(key));
    if (existingKey) {
      if (pendingReceptorAttachSourceSignal(RECEPTOR_SCORES[existingKey], candidate)) {
        summary.receptorSignalsAttached += 1;
      }
      existing.add(pendingReceptorCompareKey(key));
      continue;
    }
    if (existing.has(pendingReceptorCompareKey(key))) continue;
    RECEPTOR_SCORES[key] = pendingReceptorProfileFromCandidate(candidate);
    existing.add(pendingReceptorCompareKey(key));
    summary.receptorProfilesAdded += 1;
  }
  return summary;
})();

function pendingReceptorPrimaryDrugName(candidate = {}) {
  if (typeof pendingLivePrimaryDrugName === "function") return pendingLivePrimaryDrugName(candidate);
  return String(candidate.drug || candidate.drugs?.[0] || candidate.name || "").trim();
}

function pendingReceptorLookupKey(value) {
  return String(value || "").toLowerCase().replace(/\s+/g, "_").replace(/-/g, "").slice(0, 120);
}

function pendingReceptorCompareKey(value) {
  return String(value || "").toLowerCase().replace(/[^a-z0-9]+/g, "");
}

function pendingReceptorAttachSourceSignal(target, candidate = {}) {
  ...
  target.pendingSourceSignals.push({
    ...
    summary:String(candidate.summary || "Pending source-linked receptor/phenotype context.").slice(0, 360),
    professionalReviewStatus:"pending",
    professionallyReviewed:false,
    canAffectPublicSeverity:false,
  });
...
function pendingReceptorProfileFromCandidate(candidate = {}) {
  const text = typeof pendingLiveCandidateText === "function"
    ? pendingLiveCandidateText(candidate)
    : String([candidate.title, candidate.summary, ...(candidate.drugs || []), ...(candidate.genes || [])].join(" ")).toLowerCase();
  const profile = { SERT:0, NET:0, H1:0, M1:0, alpha1:0, hERG:0, GABA:0, muOp:0, D2:0, MAO:0, DAT:0 };
  if (/seroton|sert|ssri|snri|tramadol|linezolid/.test(text)) profile.SERT = 1;
  if (/norepinephrine| noradren| net |snri|atomoxetine|venlafaxine|duloxetine/.test(text)) profile.NET = 1;
  if (/dopamine| dat |stimulant|methylphenidate|bupropion/.test(text)) profile.DAT = 1;
  if (/histamine| h1 |sedat|somnol|sleep/.test(text)) profile.H1 = 1;
  if (/anticholin|muscarinic| m1 |deliri|oxybutynin|amitriptyline/.test(text)) profile.M1 = 1;
  if (/alpha|orthostat|hypoten|fall/.test(text)) profile.alpha1 = 1;
  if (/qt|qtc|herg|torsade|arrhythm|haloperidol|ziprasidone|amiodarone/.test(text)) profile.hERG = 1;
  if (/gaba|benzodiazepine|sedat|cns/.test(text)) profile.GABA = 1;
  if (/opioid|morphine|codeine|tramadol|oxycodone|hydrocodone|fentanyl/.test(text)) profile.muOp = 1;
  if (/dopamine| d2 |antipsychotic|aripiprazole|haloperidol|risperidone|olanzapine|quetiapine/.test(text)) profile.D2 = 1;
  if (/mao|monoamine oxidase|linezolid|selegiline|phenelzine|tranylcypromine/.test(text)) profile.MAO = 1;
  if (!Object.values(profile).some(Boolean)) {
    profile.H1 = 1;
    profile.alpha1 = 1;
  }
  profile.pendingSourceSignal = true;
  profile.reviewRequired = true;
  profile.professionalReviewStatus = "pending";
  profile.professionallyReviewed = false;
  profile.experimentalOnly = true;
  profile.canAffectPublicSeverity = false;
  profile.canBeUsedForClinicalAction = false;
  return profile;
}
