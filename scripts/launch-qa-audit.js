...
const PANEL_DEFS = [
  ...
  { name:'evidence quality', sectionId:'qualitySection', bodyId:'qualityBody' },
];
...
function collect({ name, why, drugs, genotypes = [], tab = 'genes-metabolites', expect }) {
  ...
  assert(/findings/i.test(reviewSummaryText) && /No Sign-Off Claimed/i.test(reviewSummaryText), `${name}: missing Reviewer Summary diagnostics\n${debug()}`);
